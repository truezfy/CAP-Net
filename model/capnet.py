...
1. Forward Fusion Layer: PET and CT are fused via RWKV to obtain global features, then projected to multiple scales
    10|2. UNet Encoder: Each layer receives corresponding global fusion features (addition operation)
...
wkv_cuda = None
    ...
    30|# First try to load the compiled .so file (bypass torch.utils.cpp_extension's locking mechanism)
...
if os.path.exists(_wkv_so_path):
    ...
    40|        spec = importlib.util.spec_from_file_location("wkv_cuda", _wkv_so_path)
...
class SingleLayerTokenExtractor:
    ...
    def __init__(self):
        ...
        # New: Cache GPU tensor to avoid conversion on every forward pass
   ...
   100|                if isinstance(value, torch.Tensor):
        ...
        # Clean up cache
   110|        self._cached_regions_tensor = None
        self._cached_device = None
   ...
   140|        if isinstance(token_info_data, str):
            data.close()
   ...
   150|        """
        ...
        if self._cached_regions_tensor is None or self._cached_device != device:
            ...
            if isinstance(image_regions, torch.Tensor):
                ...
                # numpy array or list
   160|                self._cached_regions_tensor = torch.tensor(image_regions, device=device, dtype=torch.float32)
   ...
   170|        # Recreate if cache doesn't exist or device changed
   ...
   180|        
    ...
    def _get_patch_sizes(self, device):
        ...
        if not hasattr(self, cache_key) or getattr(self, cache_key) is None or self._cached_device != device:
   190|            patch_sizes = self.token_info['patch_sizes']
   ...
   200|    def _compute_spatial_sort_indices(self):
...
class WKV(torch.autograd.Function):
   ...
   240|        bf_mode = (w.dtype == torch.bfloat16)
   ...
   250|        if RUN_CUDA and wkv_cuda is not None:
   ...
   280|        if RUN_CUDA and wkv_cuda is not None:
            ...
            raise RuntimeError(
   290|                "CUDA WKV backward not available, CUDA support is required to run this model.\n"
                "RUN_CUDA=False, wkv_cuda is None"
            )
   ...
   300|            gu = torch.sum(gu.bfloat16(), dim=0)
   ...
   310|        y = torch.zeros((B, T, C), device=w.device)
   ...
   340|        gv = torch.zeros((B, T, C), device='cuda')
        return gw, gu, gk, gv
...
class PositionEmbedding(nn.Module):
   ...
   370|        for i, size in enumerate(patch_sizes):
   ...
   400|        enhanced_tokens = tokens + context_features
...
class MShift(nn.Module):
   ...
   420|    def forward(self, pet_tokens: torch.Tensor, ct_tokens: torch.Tensor) -> torch.Tensor:
...
# ===== BiWKV =====
   ...
   440|    def forward(self, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        """Bidirectional WKV computation"""
        B, N, C = k.shape
   ...
   450|            u_first = self.spatial_first / N
            if w_decay.dim() == 1:
   ...
   460|            k_reversed = torch.flip(k, dims=[1])
   ...
   500|                        forward_val = self.spatial_first[c] * k[b, t, c]
...
class CAP_ChannelMix(nn.Module):
   ...
   530|        hidden_sz = int(hidden_rate * n_embd)
   ...
   540|    def forward(self, x: torch.Tensor) -> torch.Tensor:
...
class CAPRWKVBlock(nn.Module):
   ...
   610|        cap_tokens = cap_tokens + self.channel_mix(self.norm2(cap_tokens))
        return cap_tokens
...
class AdaptiveTokenExtraction(nn.Module):
    ...
    def __init__(self, feature_dim: int, embedding_dim: int):
        super().__init__()
   620|        self.feature_dim = feature_dim
   ...
   640|        - Use torch.gather to extract all token features at once (1 GPU operation)
        ...
        Args:
   650|            features: Feature map [B, C, D, H, W] - original image size (shared_enc1 has no downsampling)
            token_info: token information
        Returns:
   ...
   680|            regions_tensor = torch.tensor(image_regions, device=device, dtype=torch.float32)
   ...
   690|        center_d = torch.clamp(center_d, 0, D_feat - 1)  # [N]
   ...
   700|        linear_indices = linear_indices.unsqueeze(0).expand(B, N)  # [B, N]
   ...
   710|        # gather operation: on dim=2, extract features indexed by linear_indices from features_flat
...
class ForwardFusionMultiModalVRWKV(nn.Module):
   ...
   730|    - Project global features to multiple scales (using average pooling, similar to up1, up2, up3)
    ...
    def __init__(self, n_embd: int = 256, n_layer: int = 16, num_classes: int = 2, 
        ...
        # === Forward fusion layer: PET and CT independent encoders ===
        ...
        # Independent encoders can better extract features from each modality, avoiding performance compromises from shared encoders
        self.pet_enc1 = nn.Sequential(
   750|            nn.Conv3d(1, init_features, kernel_size=3, stride=1, padding=1),
   ...
   760|        # === Token extraction module (forward fusion layer) ===
   ...
   780|            nn.InstanceNorm3d(init_features),
            nn.ReLU(inplace=True)
        )
   ...
   820|            nn.Conv3d(init_features * 8, init_features * 4, kernel_size=1),
            nn.InstanceNorm3d(init_features * 4),
            nn.ReLU(inplace=True)
        )
   ...
   840|        self.block_2_1 = nn.Sequential(
   ...
   850|        self.pool_2 = nn.MaxPool3d(kernel_size=2, stride=2)  # Downsample to 1/4
   ...
   860|            nn.InstanceNorm3d(init_features * 4),
            nn.ReLU(inplace=True)
        )
   ...
   880|        # Decoder fusion module: fuse decoder_1 features, skip connection and CT features
   ...
   910|                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
   ...
   920|        self.token_extractor.load_token_info(token_info_path)
   ...
   930|        # No longer precompute cache, let forward pass create on demand at model's device
   ...
   950|        # Consistent with AdaptiveTokenExtraction: center = (start + end) / 2.0
   ...
   970|            center_w = torch.clamp(center_w, 0, W - 1)
  ...
  1000|        B, _, D, H, W = pet_tensor.shape
        token_info = self.token_extractor.token_info
        
        # === Forward fusion layer: PET and CT independent encoders, extract tokens, RWKV fusion ===
        pet_feat1 = self.pet_enc1(pet_tensor)  # [B, init_features, D, H, W]
        ct_feat1 = self.ct_enc1(ct_tensor)     # [B, init_features, D, H, W] - independent encoder
        
        
        
        # Extract tokens and process
  1010|        pet_tokens = self.pet_token_extractor_fusion(pet_feat1.detach(), token_info)
  ...
  1020|        # Convert RWKV fused tokens to feature map (original image size)
  ...
  1030|        # 1/4 size features (for adding to bottleneck)
  ...
  1040|        ds0 = self.fusion_conv_1(ds0)  # [B, init_features, D, H, W]
  ...
  1050|        
        ...
        # === UNet decoder (3-layer decoding) ===
        # Decoder Block 2: 1/4 -> 1/2
  1060|        x = self.upconv_2(x)  # [B, init_features*2, D/2, H/2, W/2]
  ...
  1070|        x = self.decoder_1_1(x)  # [B, init_features, D, H, W]
  ...
  1080|            if 'core_features' in self.token_extractor.token_info:
  ...
  1090|        return logits, token_logits
  ...
  1110|            return torch.empty(B, 0, num_classes, device=device)
  ...
  1120|            start_d = max(0, int(start_d))
...
def create_forward_fusion_multimodal_vrwkv_model_v7(
  ...
  1160|    print(f"    - Based on v5 architecture (independent encoders + CT fine-grained features)")
    print(f"    - PET and CT use independent encoders (avoid performance compromise from shared encoders)")
  ...
  1170|def create_forward_fusion_multimodal_vrwkv_model_v8(
    n_embd: int = 256, 
    n_layer: int = 16, 
    num_classes: int = 2, 
    init_features: int = 16
    ...
    print(f"  Number of classes: {num_classes}")
    print(f"  Architecture features:")
    print(f"    - Based on v7 architecture (independent encoders + CT fine-grained features)")
    print(f"    - PET and CT use independent encoders (avoid performance compromise from shared encoders)")
    print(f"    - CT features provide both token-level global information and fine-grained features")
    print(f"    - CT fine-grained features only added to highest-level decoder decoder_1 (original image size)")
    print(f"    - Encoder stage only fuses PET features and fused global features, without CT features")
...
if __name__ == "__main__":
  ...
  1210|    ct_input = torch.randn(B, 1, D, H, W).cuda()
